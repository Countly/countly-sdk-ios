...
class CountlyUserProfileTests: CountlyBaseTestCase {
    ...
    func test_208_CR_CG_M() {
        ...
        // Give all consent again here, else features will not work because
        // device id without merge change has cancelled all consents
        setSameData()
        Countly.sharedInstance().recordEvent("D")
    ...
    func test_209_CR_CNG_M() {
        ...
        // consents, location, device id change
        XCTAssertEqual(3, CountlyPersistency.sharedInstance().remainingRequestCount())
        guard let queuedRequests = CountlyPersistency.sharedInstance().value(forKey: "queuedRequests") as? [String]
        else {
            fatalError("Failed to get queuedRequests from CountlyPersistency")
    ...
    func validateUserDetails(request: String) {
        ...
        if userDetails != nil {
            ...
            XCTAssertEqual(
                "United Nations", userDetailsMap["organization"] as! String, "organization should be United Nations")
            XCTAssertEqual("+0123456789", userDetailsMap["phone"] as! String, "phone should be +0123456789")
            XCTAssertEqual(
                "https://s12.postimg.org/qji0724gd/988a10da33b57631caa7ee8e2b5a9036.jpg",
                userDetailsMap["picture"] as! String,
                "picture should be https://s12.postimg.org/qji0724gd/988a10da33b57631caa7ee8e2b5a9036.jpg")
