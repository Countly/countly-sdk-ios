...
- (void)presentWidget_new:(WidgetCallback)widgetCallback;
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [webViewManager createWebViewWithURL:[self generateWidgetURL]
        frame:frame
        appearBlock:^{
...
- (void)presentWithCallback:(WidgetCallback)widgetCallback;
{
  ...
  dismissButton.onClick    = ^(id sender) {
    [webVC dismissViewControllerAnimated:YES
                              completion:^{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationResponse:(WKNavigationResponse *)navigationResponse decisionHandler:(void (^)(WKNavigationResponsePolicy))decisionHandler
{
  ...
  if (statusCode >= 400)
  {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      if (self.webVC)
      {
        [self.webVC dismissViewControllerAnimated:YES
                                       completion:^{
...
- (void)getWidgetData:(void (^)(NSDictionary *__nullable widgetData, NSError *__nullable error))completionHandler
{
  ...
  NSURLSessionTask *task = [CountlyCommon.sharedInstance.URLSession dataTaskWithRequest:[self dataRequest]
...
{
  NSString *queryString = [NSString stringWithFormat:@"%@=%@&%@=%@&%@=%@&%@=%@&%@=%@&%@=%@", kCountlyQSKeySDKName, CountlyCommon.sharedInstance.SDKName, kCountlyQSKeySDKVersion, CountlyCommon.sharedInstance.SDKVersion, kCountlyFBKeyAppVersion, CountlyDeviceInfo.appVersion, kCountlyFBKeyPlatform,
                                                     CountlyDeviceInfo.osName, kCountlyFBKeyShown, @"1", kCountlyFBKeyWidgetID, self.ID];
...
- (NSURL *)generateWidgetURL
{
  ...
  NSMutableString *URL = [NSMutableString stringWithFormat:@"%@%@/%@", CountlyConnectionManager.sharedInstance.host, kCountlyEndpointFeedback, self.type];
