...
- (void)fetchContents
{
    ...
    NSURLSessionTask *dataTask = [CountlyCommon.sharedInstance.URLSession dataTaskWithRequest:[self fetchContentsRequest] completionHandler:handler];
    [dataTask resume];
}
...
- (NSString *)resolutionJson
{
    ...
    NSDictionary *resolutionDict = @{@"portrait" : @{@"height" : @(lWpH), @"width" : @(lHpW)}, @"landscape" : @{@"height" : @(lHpW), @"width" : @(lWpH)}};
