...
class CountlyViewTrackingTests: CountlyViewBaseTest {
    ...
    func testStartAndStopViewWithSegmentation() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
        ...
        validateRecordedEventSegmentations(
            forEventID: viewID ?? "",
            expectedSegmentations: ["name": "View1", "visit": 1, "key": "value", "segment": "iOS"])
    }

    func testStartViewAndStopViewWithID() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
    ...
    func testStartAndStopMultipleViewsIncludingAutoStoppedViews() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
    ...
    func testPauseAndResumeViewsForMultipleViews() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
    ...
    func testMultiplePauseAndResumeCyclesOnSameView() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
        ...
        wait(
            for: [pauseExpectation, resumeExpectation, pauseExpectation1, resumeExpectation1, stopExpectation],
            timeout: 35.0)
    ...
    func testStartViewWhileAutoViewTrackingEnabled() {
        let config = createBaseConfig()
    ...
    func testStartAndStopAutoStoppedViewWithSegmentation() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
        ...
        let stopExpectation = XCTestExpectation(
            description: "Wait for 4 seconds before stopping the auto-stopped view.")
    ...
    func testStartAutoStoppedViewAndInitiateAnother() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
    ...
    func testStartRegularViewPauseAndResumeMultipleTimesThenStop() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
    ...
    func testStopAllViewsWithSpecificSegmentation() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
    ...
    func testUpdateSegmentationMultipleTimesOnTheSameView() {
        ...
        let waitForSecondSegmentation = XCTestExpectation(
            description: "Wait for 4 seconds before adding second segmentation.")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 4) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 4) {
                Countly.sharedInstance().views().addSegmentationToView(
                    withName: "View1", segmentation: ["key2": "value2"])
        ...
        validateRecordedEventSegmentations(
            forEventID: viewID ?? "",
            expectedSegmentations: ["name": "View1", "visit": 1, "startKey": "startValue", "segment": "iOS"])
        validateRecordedEventSegmentations(
            forEventID: viewID ?? "",
            expectedSegmentations: ["name": "View1", "key1": "value1", "key2": "value2", "segment": "iOS"])
    }

    func testStartViewWithConsentNotGiven() {
        let config = createBaseConfig()
        ...
        XCTAssertEqual(
            beforeEventCount, afterEventCount, "Stopping a non-started view should not record any new event.")
    }
    ...
    func testSetAndUpdateGlobalViewSegmentationWithViewInteractions() {
        ...
        validateRecordedEventSegmentations(
            forEventID: viewID2, expectedSegmentations: ["visit": 1, "key": "value", "name": "View2", "segment": "iOS"])
        validateRecordedEventSegmentations(
            forEventID: viewID2, expectedSegmentations: ["key": "newValue", "name": "View2", "segment": "iOS"])
    }
}

class CountlyViewForegroundBackgroundTests: CountlyViewBaseTest {
    func testStartMultipleViewsMoveAppToBackgroundAndReturnToForeground() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
        ...
        // Call validateRecordedEvents to check if the events match expectations
        validateQueuedViews(
            startedEventsCount: startedQueuedEventsCount, endedEventsDurations: endedQueuedEventsDurations)
    ...
    func testStartViewBackgroundAppResumeViewWhenReturningToForeground() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
    ...
    func testAttemptToStopANonStartedView() {
        let config = createBaseConfig()
        Countly.sharedInstance().start(with: config)
    ...
    func testBackgroundAndForegroundTriggers_manualViewRestartDisabled() {
        let config = createBaseConfig()
...
class CountlyViewBaseTest: CountlyBaseTestCase {
    ...
    func validateRecordedViews(startedEventsCount: [String: Int], endedEventsDurations: [String: [Int]]) {
        ...
        guard
            let recordedEvents = CountlyPersistency.sharedInstance().value(forKey: "recordedEvents") as? [CountlyEvent]
        else {
        ...
        for (key, expectedCount) in startedEventsCount {
            ...
            XCTAssertEqual(
                ...
                "Started events count for key \(key) does not match expected count \(expectedCount)")
        }
        ...
        for (key, expectedDurations) in endedEventsDurations {
            ...
            XCTAssertEqual(
                actualDurations.count, expectedDurations.count,
            ...
            for (index, expectedDuration) in expectedDurations.enumerated() {
                ...
                XCTAssertTrue(
                    mutableActualDurations.contains(expectedDuration),
            ...
            XCTAssertTrue(
                mutableActualDurations.isEmpty,
                "Not all actual durations were matched with expected durations for key \(key)")
        }
    }

    func validateQueuedViews(startedEventsCount: [String: Int], endedEventsDurations: [String: [Int]]) {
        guard let queuedRequests = CountlyPersistency.sharedInstance().value(forKey: "queuedRequests") as? [String]
        else {
        ...
        for (key, expectedDurations) in endedEventsDurations {
            ...
            for (index, expectedDuration) in expectedDurations.enumerated() {
                XCTAssertEqual(
                    actualDurations[index], expectedDuration,
                    "Duration at index \(index) for key \(key) does not match expected duration \(expectedDuration)")
            }
        }
    }
    ...
    func validateRecordedEventSegmentations(forEventID eventID: String, expectedSegmentations: [String: Any]) {
        ...
        XCTAssertFalse(
            ...
            "No recorded events found with ID \(eventID) matching expected segmentation criteria")
        ...
        for event in filteredEvents {
            ...
            for (key, expectedValue) in expectedSegmentations {
                if let actualValue = eventSegmentations[key] {
                    let msg =
                        "Segmentation mismatch for key \(key) in recorded event"
                            + " with ID \(eventID): expected \(expectedValue),"
                            + " found \(actualValue)"
                    XCTAssertEqual("\(actualValue)", "\(expectedValue)", msg)
                } else {
    ...
    func validateQueuedEventSegmentations(forEventID eventID: String, expectedSegmentations: [String: Any]) {
        ...
        // Ensure there are events with the specified ID and segmentation criteria
        XCTAssertFalse(
            filteredEvents.isEmpty, "No queued events found with ID \(eventID) matching expected segmentation criteria")
        ...
        for event in filteredEvents {
            ...
            for (key, expectedValue) in expectedSegmentations {
                if let actualValue = eventSegmentations[key] {
                    let msg =
                        "Segmentation mismatch for key \(key) in queued event"
                            + " with ID \(eventID): expected \(expectedValue),"
                            + " found \(actualValue)"
                    XCTAssertEqual("\(actualValue)", "\(expectedValue)", msg)
                } else {
