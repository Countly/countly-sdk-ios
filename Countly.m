...
- (void)startWithConfig:(CountlyConfig *)config
{
  ...
  {
    CLY_LOG_I(@"Initializing with %@ SDK v%@ on %@ with %@ %@", CountlyCommon.sharedInstance.SDKName, CountlyCommon.sharedInstance.SDKVersion, CountlyDeviceInfo.device, CountlyDeviceInfo.osName, CountlyDeviceInfo.osVersion);
  }
  else
  {
    CLY_LOG_I(@"Initializing with %@ SDK v%@ on %@ with %@ %@ default SDK name %@ default SDK version %@", CountlyCommon.sharedInstance.SDKName, CountlyCommon.sharedInstance.SDKVersion, CountlyDeviceInfo.device, CountlyDeviceInfo.osName, CountlyDeviceInfo.osVersion, kCountlySDKName,
              kCountlySDKVersion);
  }
  ...
  if (!CountlyConsentManager.sharedInstance.consentForSessions)
  {
    ...
    if (!CountlyConsentManager.sharedInstance.consentForLocation || CountlyLocationManager.sharedInstance.isLocationInfoDisabled || !CountlyServerConfig.sharedInstance.locationTrackingEnabled)
    {
