...
class CountlyTests: CountlyBaseTestCase {
    ...
    func testReInitWithDeviceId() throws {
        ...
        XCTAssertTrue(
            Countly.sharedInstance().deviceIDType() == CLYDeviceIDType.IDFV,
            "Countly deviced id type should be IDFV when no device id is provided during init.")
        ...
        XCTAssertTrue(CountlyCommon.sharedInstance().hasStarted, "Countly initialization failed.")
        XCTAssertTrue(
            Countly.sharedInstance().deviceID() == sdkGeneratedDeviceID,
            "Countly device id not match with provided device id.")
        XCTAssertTrue(
            Countly.sharedInstance().deviceIDType() == CLYDeviceIDType.IDFV,
